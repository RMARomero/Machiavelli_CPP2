...
{
	ifstream input(m_FilePath); //create file stream object
...
void CardReader::createCard(string name, int costs, int points, string color, int ammount, bool special)
{
	for (int i = 0; i < ammount; i++) 
	{
		shared_ptr<DistrictCard> card { new DistrictCard(name, costs, points, m_ColourMap[color])};
